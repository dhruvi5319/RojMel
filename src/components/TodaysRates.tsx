 ...
 */
export function TodaysRates({ rates }: { rates: FuelRate[] }) {
  const t = useT()
  const lang = useLang()
  const [editing, setEditing] = useState<string | null>(null)

  const stale = rates.filter((r) => !r.set_today)
  const nameOf = (r: FuelRate) => (lang === 'gu' && r.name_gu ? r.name_gu : r.name)
          ...
          <div key={r.fuel_type_id} className="px-5 py-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                {r.set_today ? (
                  <Check className="size-4 text-accent-2-700" aria-label={t('rate.setToday')} />
                ) : (
              ...
              <div className="flex items-center gap-3">
                <span className="tabular font-[family-name:var(--font-heading)] text-[22px] leading-none">
                  {r.sale_rate != null ? `₹${Number(r.sale_rate).toFixed(2)}` : '—'}
                  <span className="ml-1 text-[13px] text-neutral-600">/{r.unit}</span>
                </span>
                <button
                  type="button"
                  onClick={() =>
                    setEditing(editing === r.fuel_type_id ? null : r.fuel_type_id)
                  }
            ...
            {editing === r.fuel_type_id ? (
              <div className="mt-3">
                ...
                >
                  <input type="hidden" name="fuel_type_id" value={r.fuel_type_id} />
                  <NumberInput
                    name="sale_rate"
                    step="0.001"
                    required
